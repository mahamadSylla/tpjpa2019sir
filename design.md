...
-La classe Utilisateur est reliée à la classe :
	...
	\t@ManyToMany
	@JoinTable(name = "les_absences", joinColumns = @JoinColumn (name="utilisateur_id"), inverseJoinColumns = @JoinColumn (name="reunion_id"))
	@JsonManagedReference(value = "utilisateur_absent") : pemet de retrouver les reunoins manquées par un utilisateur.
